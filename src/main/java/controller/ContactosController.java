...
public class ContactosController {
	...
	@ApiOperation(value="Añade un nuevo contacto recibido en el cuerpo de la petición")
	@PostMapping(value="contactos",consumes=MediaType.APPLICATION_JSON_VALUE,produces=MediaType.TEXT_PLAIN_VALUE)
	public String guardarContacto(@ApiParam(value = "Objeto JSON a añadir", required = true) @RequestBody Contacto contacto) {		
		return String.valueOf(service.agregarContacto(contacto));
	}
	@ApiOperation(value="Actualiza en la base de datos el contacto recibido en el cuerpo de la petición")
